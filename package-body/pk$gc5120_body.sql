create or replace PACKAGE BODY pk$gc5120 AS
 /**
  * ===========================================================<br/>
...
--  PROCEDURE pr_val (pi_key IN NUMBER);
  PROCEDURE pr_apr_prc(pi_key IN NUMBER, pi_cn IN NUMBER) IS
     BEGIN
		 pkgc_20_lnk.pr_apr_prc (pi_key  => pi_key,
								 pi_cn => pi_cn);
		pkgc_00.pr_commit;
  END pr_apr_prc;
  --
   PROCEDURE pr_rej_prc(pi_key IN NUMBER, pi_cn IN NUMBER) IS
   BEGIN
		 pkgc_20_lnk.pr_rej_prc (pi_key  => pi_key,
								 pi_cn => pi_cn);
		pkgc_00.pr_commit;
  END pr_rej_prc;
  --
